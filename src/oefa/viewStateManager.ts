...
export function extractViewStateFromHtml(html: string): string {
    ...
    if (!value) {
        throw new Error('No se encontró javax.faces.ViewState en el HTML recibido.');
    }
    return value;
}
...
export function extractViewStateFromPartialResponse(xml: string): string {
    ...
    if (!match || !match[1]) {
        logger.error('No se pudo extraer ViewState de la respuesta parcial. XML recibido (primeros 500 chars):');
